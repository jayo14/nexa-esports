  ...
  try {
    ...
    const { data: transaction, error: txError } = await supabaseAdmin
      ...
      .insert({
        ...
        metadata: { phone_number, network_provider, service_id },
      })
      .select("id")
      .single();
    ...
    try {
      ...
      await supabaseAdmin.rpc("wallet_record_provider_operation", {
        ...
        p_provider_request: { referenceNumber, amount, phone_number: normalizedPhone, network_provider, service_id },
      ...
      if (isSuccess) {
        ...
        await supabaseAdmin.rpc("wallet_enqueue_settlement", {
          ...
          p_source: "purchase_data_success",
        });
        ...
        return respond({ error: pagaData.responseMessage || "Data purchase failed", reference: referenceNumber }, 400);
      }
    } catch (pagaError) {
      ...
      await supabaseAdmin.rpc("wallet_enqueue_settlement", {
        ...
        p_source: "purchase_data_exception",
      });

      return respond({
        success: true,
        message: "Data purchase is being processed. Please check your balance in a moment.",
        reference: referenceNumber,
      }, 202);
    }
