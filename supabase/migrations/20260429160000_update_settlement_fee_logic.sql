-- Update wallet_settle_transaction to support "fee on top" logic using metadata
  ...
  IF v_tx.type = 'deposit'::transaction_type THEN
    IF p_decision = 'success' THEN
      ...
      SET wallet_state = 'success'::public.wallet_tx_state,
          settled_at = NOW(),
          settlement_source = p_source,
          -- For "fee on top", the amount in the ledger is the net amount, 
          -- but we keep the transaction amount as the total charged for accounting.
          -- Actually, to keep it consistent with UI expectations, we update amount to v_net.
          amount = v_net, 
