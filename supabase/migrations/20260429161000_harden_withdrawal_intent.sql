-- Harden wallet_create_withdrawal_intent with explicit casts and better error handling
...
BEGIN
  ...
  -- Use digest without extensions. prefix if we added extensions to search_path
  BEGIN
    v_request_hash := encode(digest(convert_to((p_user_id::text || '|' || p_amount::text || '|' || COALESCE(p_wallet_type, 'clan') || '|' || COALESCE(p_client_reference, '')), 'UTF8'), 'sha256'), 'hex');
  EXCEPTION WHEN OTHERS THEN
    -- Fallback to extensions.digest if needed
    v_request_hash := encode(public.digest(convert_to((p_user_id::text || '|' || p_amount::text || '|' || COALESCE(p_wallet_type, 'clan') || '|' || COALESCE(p_client_reference, '')), 'UTF8'), 'sha256'), 'hex');
  END;
