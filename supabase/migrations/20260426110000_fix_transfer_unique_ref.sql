...
BEGIN
  ...
  VALUES(
    ...
    amount, v_fee, 'processing', 'pending', v_ref || '_out', 'Transfer to ' || recipient_ign, NOW()
  )
  ...
  VALUES(
    ...
    amount, 0, 'processing', 'pending', v_ref || '_in', 'Transfer from ' || (SELECT ign FROM public.profiles WHERE id = sender_id), NOW()
  )
