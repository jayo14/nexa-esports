...
CREATE OR REPLACE FUNCTION public.debit_wallet(
    ...
    p_type       public.wallet_type DEFAULT 'clan'
)
RETURNS JSONB AS $$
DECLARE
    v_wallet_id    UUID;
...
FOR SELECT USING (
  EXISTS (
    ...
    WHERE tm.team_id = public.team_members.team_id AND tm.player_id = auth.uid()
  )
);

-- Team Chat Messages
DROP POLICY IF EXISTS "Team members can view chat" ON public.team_chat_messages;
CREATE POLICY "Team members can view chat" ON public.team_chat_messages
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM public.team_members tm 
    WHERE tm.team_id = public.team_chat_messages.team_id AND tm.player_id = auth.uid()
  )
);

DROP POLICY IF EXISTS "Team members can send chat" ON public.team_chat_messages;
CREATE POLICY "Team members can send chat" ON public.team_chat_messages
